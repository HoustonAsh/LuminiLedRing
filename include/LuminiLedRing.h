...
public:
  ...
  void progress(uint32_t rgb, uint8_t percentage);          // show progress bar 0-100% by "percentage" parameter and "rgb" value 
  void enable(uint32_t rgb);

  void blink(int32_t duration, int32_t interval, int count = -1);
  void load(uint32_t rgb, uint32_t duration, uint8_t taleScale);
